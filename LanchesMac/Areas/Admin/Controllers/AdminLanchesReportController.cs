...
using LanchesMac.Areas.Servicos;
   ...
   public async Task<ActionResult> LanchesCategoriaReport()
{
    ...
    // Adicione o prefixo "wwwroot" ao caminho da imagem
    foreach (DataRow row in lanches.Rows)
    {
        string imagemUrl = row["ImagemUrl"].ToString();
        row["ImagemUrl"] = Path.Combine("wwwroot", imagemUrl);
    }
